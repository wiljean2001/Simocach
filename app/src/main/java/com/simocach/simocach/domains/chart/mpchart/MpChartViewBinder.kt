...
import com.simocach.simocach.ui.components.chart.mpchart.base.IMpChartLineView
import com.simocach.simocach.util.QueueFixedLength

class MpChartViewBinder(
    private var context: Context,
    private var chartView: IMpChartLineView,
    private var colorSurface: Color = Color.Transparent,
...
//        Log.d("MpChartViewBinder", " prepareDataSets: ${datasets.size}")
    ...
    private fun prepareDataSet(modelDataSet: ModelChartDataSet, pLineData: LineData?): LineData? {/* val index = modelLineChart.getIndex(dataType)
...
//            Log.d("MpChartViewBinder", " prepareDataSet 1: ")
            ...
            val sets: MutableList<ILineDataSet> = pLineData.dataSets
            sets.add(dataSet)
...
//            Log.d("MpChartViewBinder", " prepareDataSet 2: ")
    ...
    private fun createDataSet(dataSet: ModelChartDataSet): ILineDataSet {/* val index: Int = modelLineChart.getIndex(dataType)*/
        val label: String? = dataSet.getLabel()
        val data: QueueFixedLength<Float>? = dataSet.getData()
        val entries: MutableList<Entry>
        if (data == null) {
            entries = ArrayList(dataSet.getSampleLength())
